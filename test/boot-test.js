...
describe('writeCommandLineArgs()', () => {
  ...
  it('writes proper args for IBFT', () => {
    ...
    expect(Boot.fs.writeFile).calledWith(
      '/qdata/args.txt',
      '--datadir /qdata/ethereum --gasprice 0 --txpool.pricelimit 0 --permissioned --rpc ' +
      '--rpcport 8545 --rpcaddr 0.0.0.0 --ws --wsport 8546 --wsaddr 0.0.0.0 --unlock 0 --password /qdata/ethereum/passwords.txt ' +
      '--verbosity 4 --bootnodes enode://bcdefg@1.2.3.7:30301 --syncmode full --mine --rpcapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,istanbul ' +
      '--wsapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,istanbul --istanbul.blockperiod 5 --istanbul.requesttimeout 15 ' +
      '--wsorigins=* --txpool.globalslots=128 --txpool.globalqueue 32 --cache=64 --networkid 12345'
    );
  ...
  it('writes proper args for RAFT initial node', () => {
    ...
    expect(Boot.fs.writeFile.getCall(0).args[1]).to.equal(
      '--datadir /qdata/ethereum --gasprice 0 --txpool.pricelimit 0 --permissioned --rpc ' +
      '--rpcport 8545 --rpcaddr 0.0.0.0 --ws --wsport 8546 --wsaddr 0.0.0.0 --unlock 0 --password /qdata/ethereum/passwords.txt ' +
      '--verbosity 4 --bootnodes enode://bcdefg@1.2.3.7:30301 --raft --rpcapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,raft ' +
  ...
  it('writes proper args for RAFT other node based on args', () => {
    ...
    expect(Boot.fs.writeFile.getCall(0).args[1]).to.equal(
      ...
      '--wsorigins=* --txpool.globalslots=4096 --txpool.globalqueue 1024 --cache=128 --networkid 12345');
